...
#endif
#include <M5GFX.h>
#if defined( ENABLE_GESTURE ) //2024.06.23 追加
#include "Gesture.h"
...
//カメラモジュールの GPIOピンアサイン等を保存しておく構造体
static camera_config_t camera_config = {
    .pin_pwdn     = -1,
    .pin_reset    = -1,
    .pin_xclk     = 2,  //クロック信号
    .pin_sscb_sda = 12, //SCCBインターフェースのSDA信号
    .pin_sscb_scl = 11, //SCCBインターフェースのSCL信号
    ...
    .pin_vsync = 46,//垂直同期信号
    .pin_href  = 38,//水平同期信号
    .pin_pclk  = 45,//Pixel Clock

    .xclk_freq_hz = 20000000,//20MHzのクロックパルス
    .ledc_timer   = LEDC_TIMER_0,
    .ledc_channel = LEDC_CHANNEL_0,

    //PIXFORMAT_RGB565: 1pixel のデータは2byteデータ。
    //水平１行の画素のデータサイズは、320 x 2 = 640 byte
    .pixel_format = PIXFORMAT_RGB565,
    .frame_size   = FRAMESIZE_QVGA,   // QVGA(320x240)
    .jpeg_quality = 0,
    .fb_count     = 2,
...
esp_err_t camera_capture(){
  ...
  //M5.Display.setAddrWindow(0, 0, LCD_WIDTH/4, LCD_HEIGHT/4);//2024.06.23 
...
#if defined( ENABLE_GROVE_BUTTON ) //2024.05.15 追加
  //2024.05.20 Button unit [U027]でシャットダウン
  uint PB_IN0_pressed_millis = 0;
...
{
  vTaskDelay(3000);//音声応答終了終了時にHexLEDが光る（回転）処理がある。この終了を待つ。
...
{
  if(str.find(LedPictWord[0].c_str()) != string::npos) hex_led_ptn_RAIN();//雨
  else if(str.find(LedPictWord[1].c_str()) != string::npos) hex_led_ptn_FLOWER();//"花"
  ...
  else if(str.find(LedPictWord[9].c_str()) != string::npos) hex_led_ptn_UFO();//"宇宙船"
  else if(str.find(LedPictWord[10].c_str()) != string::npos) hex_led_ptn_ROBOT();//"ロボット"
  else if(str.find(LedPictWord[11].c_str()) != string::npos) hex_led_ptn_SQUID();//"イカ"
  else if(str.find(LedPictWord[12].c_str()) != string::npos) hex_led_ptn_CAFFEE();//"コーヒー"
  else if(str.find(LedPictWord[13].c_str()) != string::npos) hex_led_ptn_CAFFEE();//"珈琲"
  else if(str.find(LedPictWord[14].c_str()) != string::npos) hex_led_ptn_MUSHROOM();//"きのこ"
  else if(str.find(LedPictWord[15].c_str()) != string::npos) hex_led_ptn_SQUID();//"烏賊"
  else if(str.find(LedPictWord[16].c_str()) != string::npos) hex_led_ptn_ALIAN();//"宇宙人"
  else if(str.find(LedPictWord[17].c_str()) != string::npos) hex_led_ptn_JPN();//"日本"
  else hex_led_ptn_off();
}
#endif  //ENABLE_HEX_LED
#pragma endregion
...
int time_shutdown = 0;  //実行までの時間[minute]
void task_shutdown(void *args)
{
  if(time_shutdown == 0) 
    Voicevox_tts((char*)"直ちにシャットダウンします。", (char*)TTS_PARMS.c_str());
  else {
    char buff[100];
    sprintf(buff,"%d分後にシャットダウンします。",time_shutdown);
    Voicevox_tts(buff, (char*)TTS_PARMS.c_str());
    vTaskDelay(time_shutdown * 60 * 1000);  
  }
  ...
  vTaskDelete(NULL);//このタスク（void task_shutdown）を終了する。
}

void shutdown(int time_minute)
{
  Serial.print("shutdown time = ");
  Serial.println(time_minute);
  
  time_shutdown = time_minute;//実行までの時間[minute]
  //task_shutdownをタスクとして起動する。
  avatar.addTask(task_shutdown,"task_shutdown");
}
#pragma endregion

#pragma region //ジェスチャー
#if defined( ENABLE_GESTURE)
paj7620 Gesture;
#endif  //ENABLE_GESTURE 

#pragma endregion //ジェスチャー
...
void handle_speech() {
  ...
  Serial.println("handle_speech() Voicevox_tts(message)");
...
String payload1 = "{\"messages\": [{\"role\": \"system\", \"content\": \"" + prompt  + "\"},"
  ...
  + "\"functions\": ["
        ...
        "{"
            "\"name\": \"shutdown\","
            "\"description\": \"システムを即時終了します。実行までの時間を分単位で指定することもできます。\","
            "\"parameters\": {"
              "\"type\":\"object\","
              "\"properties\": {"
                  "\"time_shutdown\":{"
                    "\"type\": \"integer\","
                    "\"description\": \"実行までの時間。単位は分。指定がなければ即時実行。\""
                  "}"
              "},"
              "\"required\": [\"\"]"
            "}"
  ...
  if(ret != ""){
    ...
    if (error) {
      ...
      if(data == NULL)//function callingの場合は"content"には何もない(null)
      {
        ...
        {
          String Params = params;     //{"time_shutdown":5}
          Params = Params.substring(Params.indexOf(":")+1);
          Params.replace("/[^0-9]/","");
          Serial.print("chatGpt function name = shutdown, time_shutdown = ");
          Serial.println(Params);// 3
          int iParam1 = atoi(Params.c_str());
          Serial.print("chatGpt function time_shutdown -> iParam1 = ");
          Serial.println(iParam1);;// 数値3
          shutdown(iParam1);//M5Stackをシャットダウンする。
        }
...
int random_words_max = 0;
#pragma endregion  //従来の独り言キーワード

//2024.07.06
#pragma region //新しい独り言キーワード（5W1H型）
String wd_when[] = {"今日","昨日","明日","去年","来年","過去","未来","太古","朝","昼","夜","一昔前","昔々","","MAX"};//、
String wd_where[] = {"日本","世界","地球","宇宙","アジア","欧州","アフリカ","アメリカ","南極","北極","天の川銀河","外宇宙","陸","森","海","深海","空","町","田舎","都市","庭","","MAX"};//で
String wd_who[] = {"人","宇宙人","日本人","外国人","犬","猫","魚","猿","タコ","イカ","神","仏","ウイルス","MAX"};//が
String wd_what[] = {"経済","社会","食材","科学","生物","ロボット","星","空間","概念","虫","知性","MAX"};//を
String wd_why[] = {"怖い","寒い","熱い","興味本位","楽しい","悲しい","疲れた","美しい","可愛らしい","","MAX"};//から
String wd_how[] = {"発展させた","売却した","創った","壊した","買った","捨てた","分解した","研究した","食べた","教えた","料理した","MAX"};//～する（した）話を
String wd_style[] = {"ドキュメンタリー","ニュース","史実","漫才","昔話","ドラマ","エンタメ","民話","おとぎ話","MAX"};//風に語って
int wd_when_max = 0;
int wd_where_max = 0;
int wd_who_max = 0;
int wd_what_max = 0;
...
string task_Voicevox_tts_speech_text_buffer;
#pragma endregion //新しい独り言キーワード（5W1H型）


//独り言キーワードの数を数える。（文字列配列の最後は""MAX"とする）
int indexOfString(String *str)
{
  int i = 0;
  for(;i<30;i++) { if(str[i] == "MAX") break; }
  if(i>0) i--;
  return i;
}
...
void setup()
{
  ...
  //2024.07.06
  wd_when_max = indexOfString(wd_when);
  wd_where_max = indexOfString(wd_where);
  wd_who_max = indexOfString(wd_who);
  wd_what_max = indexOfString(wd_what);
  wd_why_max = indexOfString(wd_why);
  wd_how_max = indexOfString(wd_how);
  wd_style_max = indexOfString(wd_style);

  random_words_max = indexOfString(random_words);

  LedPictWord_max = indexOfString(wd_style);
  ...
  task_Voicevox_tts_speech_text_buffer = "";
  millis_period_of_inactivity = 0;
  auto cfg = M5.config();
  //2024.06.26 コメントアウトした。そうしないとPortBに接続したGrove BUTTONが正常に動作しない。
...
//  M5.Speaker.setVolume(200);
  ...
  //2024.05.09  独り言が同じになりやすいことへの対策
  randomSeed(t.seconds);

  //==============================================
  //ENABLE_GESTURE と　ENABLE_HEX_LED　は両立しない
  //==============================================
...
#if 0//現在未使用
#define ELEMENTS_OF_RANDOM_2ND_WORDS 5
String random_2nd_words[ELEMENTS_OF_RANDOM_2ND_WORDS] = {
  "もう少し詳しく説明して下さい。", //1
  "それを何かに例えて説明して下さい。", //2
  "軽い口調で言い直して下さい。", //3
  "別の見方があれば述べて下さい。"//4
...
void task_Voicevox_tts(void *args)
{
  string str = task_Voicevox_tts_speech_text_buffer.c_str();
  //"。"で文章を一文づつに分解し、一文話す毎に間を置く。
  vector<string> elements = split(str,"。");
  for(string x : elements){
    wordToLedPanel(x);
    Voicevox_tts((char*)x.c_str(), (char*)TTS_PARMS.c_str());
    while(mp3->isRunning()) vTaskDelay(500);
    //vTaskDelay(10000);
    //while (mp3->isRunning() && !mp3->loop()) {vTaskDelay(1);}
    //mp3->stop();
  }
  vTaskDelay(1000);
  task_Voicevox_tts_speech_text_buffer = "";
...
void cmd_Voicevox_tts(string str)
{
  bool flag_ignore = false;

  //発話中
  if(task_Voicevox_tts_speech_text_buffer != "") {
    Serial.println("cmd_Voicevox_tts : task running");
    flag_ignore = true;
  }

  //不適切発言（主に"神が・・生物を・・壊す"等が問題）はchatGPTが却下する。
  //その説明文言は再生せずに、ここで消してしまう。
  if((str.find("申し訳")==0) || (str.find("もうしわけ")==0) || (str.find("すみません")==0) ) {
    Serial.println("cmd_Voicevox_tts : futekisetu");
    speech_text_buffer ="";//却下する
    //task_Voicevox_tts_speech_text_buffer = "";
    flag_ignore = true;
  }
  ...
  //task_Voicevox_ttsをタスクとして起動する。
	task_Voicevox_tts_speech_text_buffer = str;
  Serial.println("cmd_Voicevox_tts : avatar.addTask()");
  avatar.addTask(task_Voicevox_tts,"task_Voicevox_tts");
}

void loop()
{
#if defined( ENABLE_GESTURE)
servo_home = true;//ジェスチャー有効のときはランダムなサーボ動作は止めておく
#endif
  ...
  if ((random_time >= 0) && (millis() - lastms1 > random_time))
  {
    ...
    lastms1 = millis();
    random_time = MILLIS_MONOLOGUE_SHORT_INTERVAL + 1000 * random(30);
    if (!mp3->isRunning() && (speech_text=="") && (speech_text_buffer == "") && (task_Voicevox_tts_speech_text_buffer == "")) {
    ...
    hex_led_ptn_notification();
#endif
      /////////////////////
      /* 話題とは別に話し方などの注文を追加する仕組みを入れてみた。→　一定の面白さはあったが却下。
      if((count_monologue > 1) && ((double)random(100))/100.0 > 0.5)
        exec_chatGPT(random_2nd_words[random(ELEMENTS_OF_RANDOM_2ND_WORDS)]);//2024.06.20 <==== random_2nd_words[]の要素数
      else
     ...
     {
        int str_when_i = random(wd_when_max);
        int str_where_i = random(wd_where_max);
        int str_why_i = random(wd_why_max);
        String str_when = wd_when[str_when_i];
        String str_where = wd_where[str_where_i];
        String str_why = wd_why[str_why_i];
        if(str_when != "") str_when = str_when + String("、");
        if(str_where != "") str_where = str_where + String("で");
        if(str_why != "") str_why = str_why + String("から");

        String str = str_when
                    + str_where
                    + wd_who[random(wd_who_max)] + String("が")
                    + wd_what[random(wd_what_max)] + String("を")
                    + str_why
                    + wd_how[random(wd_how_max)] + String("話を")
                    + wd_style[random(wd_style_max)] + String("風に語って");
...
#if defined(ARDUINO_M5STACK_Core2) || defined( ARDUINO_M5STACK_CORES3 )
  {
    ...
    if (count ||  flag_talk)
    {
      ...
      if (t.wasPressed() || flag_talk)
      {          
        //Serial.println("in : if (t.wasPressed()){}");
        if ((box_stt.contain(t.x, t.y)&&(!mp3->isRunning())) || flag_talk)
        {//画面上のボタンを押して音声認識させる
...
#endif
  //========================================================================
  //ジェスチャーに合わせて頭を振る。例）センサの前で手を右から左へ　→　頭も左に振る
  //========================================================================
...
servo_home = true;
    ...
    if(speech_text != ""){
      ...
      cmd_Voicevox_tts(speech_text_buffer.c_str());
    }
    //発話終了したのでMP3停止
    if (mp3->isRunning()) {
      if (!mp3->loop()) {
